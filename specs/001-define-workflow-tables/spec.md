...
- Q: Should updates allow multiple active current rows or enforce one active current row per key? → A: Enforce exactly one current row in the primary table per business key and write the pre-update row to `_Hist` on update.
...
- Q: Confirm active/deleted semantics and lifecycle behavior? → A: Active means `DeleteInd=0` with `EffToDateTime=9999-01-01 00:00:00`; delete sets `DeleteInd=1` on the single primary row after snapshotting the prior state to `_Hist`; restoration updates that primary row back to active state while preserving prior states in history.
...
**Independent Test**: Can be fully tested by creating a workflow, updating its description/state, and verifying one current primary-table record plus correctly timestamped historical snapshots.

**Acceptance Scenarios**:

1. **Given** no existing workflow with a name, **When** a user creates a workflow with required fields, **Then** the system stores one current primary-table record with control columns and no delete marker.
2. **Given** an existing active workflow, **When** the workflow description is updated, **Then** the previous version is captured in the history set and the primary table still reflects only the latest current version.
...
- A restoration is requested for a row previously marked deleted.
...
- **FR-002**: System MUST maintain both current and history datasets for each in-scope table, where the primary dataset stores only the current version and the history dataset stores prior snapshots.
...
- **FR-019**: System MUST enforce exactly one current primary-table row per business key and, on update, copy the pre-update row into the matching `_Hist` table with a closing timestamp before updating the primary row in the same transaction.
...
- **FR-025**: System MUST treat a logical delete as a state transition that first snapshots the current primary row into `_Hist`, then sets `DeleteInd=1` and closes the single primary row by setting `EffToDateTime` to the deletion timestamp.
- **FR-026**: System MUST treat restoration of a previously deleted business key as an update to the existing primary row that resets `DeleteInd=0`, sets `EffFromDateTime` to the restoration timestamp, and sets `EffToDateTime=9999-01-01 00:00:00` after preserving the prior deleted state in `_Hist`.
- **FR-027**: System MUST NOT create additional primary-table rows during restoration; prior versions remain preserved only in history.
...
- Snapshot behavior follows a slowly changing dimension style where the previous current row is copied to history before the primary row is updated in place.
...
- Historical reads return closed versions from `_Hist` by default; the primary table returns only the current row.
...
- **SC-002**: 100% of successful updates result in one new historical snapshot and one remaining current primary-table version for the affected entity.
- **SC-006**: 100% of tested updates copy the pre-update primary row to `_Hist` with a closing timestamp before the primary row is updated.
